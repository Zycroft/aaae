...
- [x] **SERV-04**: `POST /api/chat/card-action` accepts `{ conversationId, cardId, userSummary, submitData }`, validates `submitData.action` against allowlist, forwards to Copilot, returns normalized messages
...
- [x] **SERV-07**: Card action allowlist enforced server-side — rejects requests with disallowed `action` values before forwarding to Copilot
- [x] **SERV-08**: `Action.OpenUrl` domain allowlist enforced server-side
...
- [x] **SERV-12**: Unit tests for the card action allowlist validator

### Client — Chat UI

- [x] **UI-01**: Responsive layout using CSS grid/flex: split-pane (transcript + metadata drawer) on desktop ≥768px, stacked single column on mobile ≤767px; verified at 360px, 768px, 1280px
...
- [x] **UI-06**: Adaptive Cards rendered via custom `useRef`/`useEffect` wrapper around the `adaptivecards` v3 JS SDK (not `adaptivecards-react`)
- [x] **UI-07**: `AdaptiveCardMessage` component hooks `onExecuteAction`, extracts `action.data`, and calls `/api/chat/card-action` with `userSummary` derived from `cardId`
- [x] **UI-08**: Submitted Adaptive Card is immediately disabled and shows a pending state; cannot be resubmitted
...
- [x] **UI-10**: Transcript chip distinguishes user-typed messages from card-submit summaries
...
- [x] **UI-13**: Dark/light theme toggle; theme persisted to localStorage
- [x] **UI-14**: Fluid typography using CSS `clamp()`; spacing scale tokens in CSS custom properties
- [x] **UI-15**: `prefers-reduced-motion` respected — transitions/animations disabled when set
- [x] **UI-16**: ARIA live region on transcript (`aria-live="polite"`) so screen readers announce new messages
- [x] **UI-17**: All interactive elements keyboard-navigable with visible focus states
...
| UI-14 | Phase 3 | Complete |
| UI-15 | Phase 3 | Complete |
| UI-16 | Phase 3 | Complete |
| UI-17 | Phase 3 | Complete |
