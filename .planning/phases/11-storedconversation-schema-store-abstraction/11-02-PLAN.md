...
Note: The `logger` import in factory.ts must match the existing server logger pattern. Check the server logger path with: `grep -r "export.*logger" /Users/zycroft/Documents/PA/aaae/server/src/ --include="*.ts" | head -5` and adjust the import path if it differs from `'../logger.js'`.
  </action>
  <verify>
